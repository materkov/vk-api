...
function TestCase($name, $callable) {
    $callable();
}
...
function Test_App_OrdersList()
{
    ...
    $id2 = Store_CreateOrder($app['db'], "2.00", 11, "3", "4");
    $id3 = Store_CreateOrder($app['db'], "3.00", 12, "5", "6");
...
function TestApp_Order_Execute()
{
    ...
    TestCase("normal flow", function () use (&$app, $user2Id, $orderId) {
        $res = App_Order_Execute($app, $user2Id, $orderId);
        if ($res != APP_OK) {
            die("res should be APP_OK, got $res");
        }

        $trans = Store_GetTransactionByOrderId($app['db'], $orderId);
        if ($trans['order_id'] !== $orderId) {
            die("transaction order_id should be $orderId, got {$trans['order_id']}");
        }
        if ($trans['sum'] !== '0.88') {
    ...
    TestCase("transaction already created", function () use (&$app, $user2Id, $orderId) {
        Store_UpdateOrderDone($app['db'], $orderId, false);
        $res = App_Order_Execute($app, $user2Id, $orderId);
        if ($res != APP_ERR_ORDER_ALREADY_EXECUTED) {
            die("res should be APP_ERR_ORDER_ALREADY_EXECUTED, got $res");
        }
